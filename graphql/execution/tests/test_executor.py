...
async def test_executes_arbitary_code():
    ...
    result = await execute(schema, ast, Data(), operation_name='Example', variable_values={'size': 100})
    assert not result.errors
    assert result.data == expected


async def test_merges_parallel_fragments():
    ast = parse('''
        { a, ...FragOne, ...FragTwo }
...
async def test_threads_root_value_context_correctly():
    doc = 'query Example { a }'
    ...
    result = await execute(GraphQLSchema(Type), ast, Data(), operation_name='Example')
    assert not result.errors
    assert resolver.got_here


async def test_correctly_threads_arguments():
    doc = '''
        query Example {
    ...
    result = await execute(GraphQLSchema(Type), doc_ast, None, operation_name='Example')
    assert not result.errors
    assert resolver.got_here


async def test_nulls_out_error_subtrees():
    doc = '''{
        ok,
        error
    }'''
    ...
    result = await execute(GraphQLSchema(Type), doc_ast, Data())
...
async def test_uses_the_inline_operation_if_no_operation_name_is_provided():
    doc = '{ a }'
    ...
    result = await execute(GraphQLSchema(Type), ast, Data())
    assert not result.errors
    assert result.data == {'a': 'b'}


async def test_uses_the_only_operation_if_no_operation_name_is_provided():
    doc = 'query Example { a }'
...
async def test_uses_the_named_operation_if_operation_name_is_provided():
    ...
    result = await execute(GraphQLSchema(Type), ast, Data(), operation_name='OtherExample')
    assert not result.errors
    assert result.data == {'second': 'b'}


async def test_raises_if_no_operation_is_provided():
...
async def test_raises_if_no_operation_name_is_provided_with_multiple_operations():
...
async def test_raises_if_unknown_operation_name_is_provided():
    doc = 'query Example { a } query OtherExample { a }'
...
async def test_uses_the_query_schema_for_queries():
    ...
    result = await execute(GraphQLSchema(Q, M, S), ast, Data(), operation_name='Q')
    assert not result.errors
    assert result.data == {'a': 'b'}


async def test_uses_the_mutation_schema_for_queries():
    ...
    result = await execute(GraphQLSchema(Q, M), ast, Data(), operation_name='M')
    assert not result.errors
    assert result.data == {'c': 'd'}


async def test_uses_the_subscription_schema_for_subscriptions():
    ...
    result = await execute(GraphQLSchema(Q, subscription=S), ast, Data(), operation_name='S')
    assert not result.errors
    assert result.data == {'a': 'b'}


async def test_avoids_recursion():
    doc = '''
        query Q {
            a
            ...Frag
    ...
    result = await execute(GraphQLSchema(Type), ast, Data(), operation_name='Q')
    assert not result.errors
    assert result.data == {'a': 'b'}


async def test_does_not_include_illegal_fields_in_output():
    ...
    result = await execute(GraphQLSchema(Q, M), ast)
    assert not result.errors
    assert result.data == {}


async def test_does_not_include_arguments_that_were_not_set():
...
async def test_fails_when_an_is_type_of_check_is_not_met():
    ...
    result = await execute(schema, query, value)
...
async def test_fails_to_execute_a_query_containing_a_type_definition():
    query = parse('''
    { foo }
...
async def test_exceptions_are_reraised_if_specified(mocker):
...
async def test_middleware_class():
    doc = '''{
        ok
        not_ok
    }'''
...
async def test_middleware_skip_promise_wrap():
    doc = '''{
        ok
        not_ok
    }'''
